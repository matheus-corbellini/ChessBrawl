document.addEventListener("DOMContentLoaded", () => {
  ...
  document.getElementById("form-jogador").addEventListener("submit", (e) => {
    ...
    if (
      jogadores.some((j) => j.nickname.toLowerCase() === nickname.toLowerCase())
    ) {
  ...
  document.getElementById("sortear-partidas").addEventListener("click", () => {
    ...
    mostrarMensagem(
      `Fase ${faseAtual} iniciada com ${rodadas.length} partidas!`,
      "success"
    );
  });

  function atualizarUIRodadas() {
    UI.rodadasDiv.innerHTML = rodadas
      .map(
        (partida) => `
  ...
  window.aplicarEvento = function (evento) {
    ...
    if (eventosPorRodada[nickname] && eventosPorRodada[nickname][evento] >= 1) {
      mostrarMensagem(
        `Cada jogador só pode usar "${evento}" uma vez por rodada!`,
        "error"
      );
      return;
    }
    ...
    mostrarMensagem(
      `${jogador.nome}: ${evento} (${eventosConfig[evento] > 0 ? "+" : ""}${
        eventosConfig[evento]
      } pts)`,
      "info"
    );
  ...
  function verificarVencedor() {
    ...
    if (jogadorA.pontos > jogadorB.pontos) {
      ...
      vencedor.pontos += 2;
      mostrarMensagem(
        `EMPATE! ${vencedor.nome} ganhou +2 pts na Blitz Match!`,
        "info"
      );
    }
  ...
  function verificarFase() {
    if (rodadas.every((p) => p.concluida)) {
      const jogadoresClassificados = jogadores.filter(
        (j) => j.vitorias >= faseAtual
      );

      if (jogadoresClassificados.length >= 2) {
        faseAtual++;
        mostrarMensagem(
          `Fase ${faseAtual} desbloqueada! Clique em "Sortear Partidas".`,
          "info"
        );
  ...
  window.aplicarEvento = function (evento) {
    ...
    if (eventosPorRodada[nickname]?.[evento] >= 1) {
      mostrarMensagem(
        `Cada jogador pode usar "${getEventoNome(
          evento
    ...
    mostrarMensagem(
      `${jogador.nome}: ${getEventoNome(evento)} (${
        pontosEvento[evento] > 0 ? "+" : ""
      }${pontosEvento[evento]} pts)`,
